...
CREATE VIEW `vw_latest_results` AS select `lr`.`Name` AS `Name`,`lr`.`BSE Code` AS `BSE Code`,`lr`.`NSE Code` AS `NSE Code`,`lr`.`Industry` AS `Industry`,`lr`.`Current Price` AS `Current Price`,`lr`.`Market Capitalization` AS `Market Capitalization`,`lr`.`Market Cap` AS `Market Cap`,`lr`.`Promoter holding` AS `Promoter holding`,`lr`.`Price to Earning` AS `Price to Earning`,`lr`.`Industry PE` AS `Industry PE`,`lr`.`Return over 1week` AS `Return over 1week`,`lr`.`Return over 1month` AS `Return over 1month`,`lr`.`Return over 3months` AS `Return over 3months`,`lr`.`Return over 6months` AS `Return over 6months`,`lr`.`EPS latest quarter` AS `EPS latest quarter`,`lr`.`EPS preceding quarter` AS `EPS preceding quarter`,`lr`.`EPS preceding year quarter` AS `EPS preceding year quarter`,`lr`.`Debt` AS `Debt`,`lr`.`OPM latest quarter` AS `OPM latest quarter`,`lr`.`OPM preceding quarter` AS `OPM preceding quarter`,`lr`.`Return on equity` AS `Return on equity`,`lr`.`Return on capital employed` AS `Return on capital employed`,`lr`.`Price to book value` AS `Price to book value`,`lr`.`FII holding` AS `FII holding`,`lr`.`DII holding` AS `DII holding`,`lr`.`Public holding` AS `Public holding`,`lr`.`Last result date` AS `Last result date`,`lr`.`screener` AS `screener`,`mct`.`mc essentials` AS `mc essentials`,`mct`.`mc technicals` AS `mc technicals`,`mm`.`Margin` AS `Margin` from ((`latest_results` `lr` left join `mc_technicals` `mct` on((`lr`.`Name` = `mct`.`Name`))) left join `mstock_margin` `mm` on((`mm`.`Symbol` = `lr`.`NSE Code`)));

DROP VIEW vw_screeners;
CREATE VIEW `vw_screeners` AS select `scr`.`Name` AS `Name`,`scr`.`BSE Code` AS `BSE Code`,`scr`.`NSE Code` AS `NSE Code`,`scr`.`Industry` AS `Industry`,`scr`.`Current Price` AS `Current Price`,`scr`.`Market Capitalization` AS `Market Capitalization`,`scr`.`Market Cap` AS `Market Cap`,`scr`.`Promoter holding` AS `Promoter holding`,`scr`.`Price to Earning` AS `Price to Earning`,`scr`.`Industry PE` AS `Industry PE`,`scr`.`Return over 1week` AS `Return over 1week`,`scr`.`Return over 1month` AS `Return over 1month`,`scr`.`Return over 3months` AS `Return over 3months`,`scr`.`Return over 6months` AS `Return over 6months`,`scr`.`EPS latest quarter` AS `EPS latest quarter`,`scr`.`EPS preceding quarter` AS `EPS preceding quarter`,`scr`.`EPS preceding year quarter` AS `EPS preceding year quarter`,`scr`.`Debt` AS `Debt`,`scr`.`OPM latest quarter` AS `OPM latest quarter`,`scr`.`OPM preceding quarter` AS `OPM preceding quarter`,`scr`.`Return on equity` AS `Return on equity`,`scr`.`Return on capital employed` AS `Return on capital employed`,`scr`.`Price to book value` AS `Price to book value`,`scr`.`FII holding` AS `FII holding`,`scr`.`DII holding` AS `DII holding`,`scr`.`Public holding` AS `Public holding`,`scr`.`screener` AS `screener`,`mct`.`mc essentials` AS `mc essentials`,`mct`.`mc technicals` AS `mc technicals`,`mm`.`Margin` AS `Margin` from ((`screeners` `scr` left join `mc_technicals` `mct` on((`scr`.`Name` = `mct`.`Name`))) left join `mstock_margin` `mm` on((`mm`.`Symbol` = `scr`.`NSE Code`)));
