...
def extract_mc_technical(driver, url, stock_name='UNKNOWN', cookies_path='cookies.txt'):
    ...
    time.sleep(1)
    filename = f"datafiles/webpages/{stock_name}_daily_technicals.html"
    with open(filename, "w", encoding="utf-8") as f:
        f.write(driver.page_source)
    print(f"💾 Saved technicals HTML to {filename}")




def download_pages(technical_url, stock_name, folder="datafiles/webpages/"):
    os.makedirs(folder, exist_ok=True)

    technical_file = os.path.join(folder, f"{stock_name}_daily_technicals.html")
    ...
    try:
        ...
        print(f"🌐 Downloading Technical file for {stock_name}")


        extract_mc_technical(driver, technical_url, stock_name=stock_name)
...
while True:
    try:
        ...
        for name, urls in urls_to_download.items():
            ...
            technical_url = urls.get('technicals')
            print(f"🔍 Checking: {name} at {datetime.now().strftime('%H:%M:%S')}")
            download_pages(technical_url, name)
