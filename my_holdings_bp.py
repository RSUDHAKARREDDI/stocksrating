...
def my_holdings_view(holding_id):
    with engine.begin() as conn:
        row = conn.execute(
            text("SELECT * FROM my_holdings WHERE holding_id=:id"),
            {"id": holding_id}
        ).mappings().first()
        # fetch basket name if you want to display it
        bname = None
    ...
    if not row:
        ...
        return redirect(url_for("my_holdings_bp.my_holdings_list"))
    return render_template("my_holdings/my_holdings_view.html", row=row, basket_name=bname)
...
def my_holdings_edit(holding_id):
    ...
    with engine.begin() as conn:
        row = conn.execute(text("SELECT * FROM my_holdings WHERE holding_id=:id"), {"id": holding_id}).mappings().first()
    if not row:
        flash("Record not found.", "warning")
        return redirect(url_for("my_holdings_bp.my_holdings_list"))
