(function(){
  ...
  // Helper to get index by header name to ensure accuracy across templates
  ...
  rows.forEach(tr => {
    ...
    // Helper for simple single-column numeric checks
    ...
    // Restore 52-Week Distance Display
    ...
    if (distContainer) {
      ...
      if (p && h) distContainer.querySelector('.dist-high').textContent = `${((p - h) / h * 100).toFixed(1)}% off High`;
      if (p && l) distContainer.querySelector('.dist-low').textContent = `+${((p - l) / l * 100).toFixed(1)}% from Low`;
    }
  });
}
