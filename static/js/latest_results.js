(function(){
  ...
  /** * CRITICAL: Index Mapping (Matches your HTML <th> order exactly)
   */
  const idx = {
    name: 0,
    industryGroup: 3, // Hidden
    industry: 4,
    price: 5,
    mcap_val: 6,
    mcap_text: 7,
  ...
  function applyHighlights() {
    ...
    rows.forEach(tr => {
      ...
      check(c[idx.pegr]?.textContent, n => n > 0 && n < 1, idx.pegr); // Highlight PEG
  ...
  function applyFilters(){
    const f = {
      ...
      scrs: new Set(Array.from($("#f_scr_multi").selectedOptions).map(o=>o.value.toLowerCase())), // Added Screener
      ...
      ranges: {
        ...
        r1w: [parseFloat($("#f_r1w_min").value), parseFloat($("#f_r1w_max").value)], // Added 1W
    ...
    $$("#resultsTable tbody tr").forEach(tr=>{
      ...
      const ok =
        ...
        (f.scrs.size === 0 || f.scrs.has(c[idx.scr].textContent.toLowerCase())) && // Fixed Screener Filter
        ...
        inRange(c[idx.r1w].textContent, f.ranges.r1w) && // Fixed 1W Filter
  ...
  $("#btn_download_csv").onclick = () => {
    ...
    rows.forEach(tr => {
      if (tr.parentElement.tagName === 'THEAD' || tr.style.display !== "none") {
        const rowData = visibleCols.map(i => {
          let text = tr.children[i].innerText.split('\n')[0].trim().replace(/"/g, '""');
          return `"${text}"`;
        }).join(",");
        csvContent += rowData + "\r\n";
      }
