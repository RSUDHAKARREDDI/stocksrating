(function(){
  ...
  // Column indices (match THEAD order)
  const idx = {
  ...
  function applyHighlights(){
    ...
    rows.forEach(tr=>{
      ...
      // 3) Returns: r1w>0, r1m>10, r3m>10, r6m>0
  ...
  // Populate multi-selects from the table values
  ...
  const baseInputs = [
    F.name, F.industryMulti, F.mcap_multi, F.mc_tech_multi, F.scr_multi,
    F.pe_min, F.pe_max
  ].filter(Boolean);
  ...
  function applyFilters(){
    ...
    const scrSel  = new Set(selectedValues(F.scr_multi));
    ...
    $$("#resultsTable tbody tr").forEach(tr=>{
      ...
      const scrOk  = scrSel.size === 0 || scrSel.has(scrVal);
      ...
      const ok = nameOk && scrOk && industryOk && peOk && roeOk && roceOk && essOk && mcapOk && techOk;
...
(function(){
  ...
  const toggleBtn  = document.getElementById('btn_toggle_filters'); // inside panel
  const handleBtn  = document.getElementById('btn_filters_handle'); // floating handle
  ...
  // Inside-panel button hides/shows the whole panel
  ...
  // Floating handle shows the panel back
  if (handleBtn) {
