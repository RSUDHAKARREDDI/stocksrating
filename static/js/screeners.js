(function(){
  ...
  // Helper to dynamically find indices based on header names for consistency
  ...
  rows.forEach(tr => {
    ...
    [idxMap.eps_lq, idxMap.eps_pq, idxMap.eps_pyq].forEach(i => {
      if (i !== -1 && c[i]) c[i].classList.add(epsOk ? "bg-good" : "bg-bad");
    });
